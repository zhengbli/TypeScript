...
namespace ts.server {
    ...
    export namespace CommandNames {
        ...
        export const GetActionsForRefactor: protocol.CommandTypes.GetRefactorCodeActions = "getRefactorCodeActions";
    }
    ...
    export class Session implements EventSender {
        ...
        private getActionsForRefactor(args: protocol.Refactor.GetRefactorCodeActionsRequestArgs): protocol.CodeAction[] {
            const { file, project } = this.getFileAndProjectWithoutRefreshingInferredProjects(args);
            const scriptInfo = project.getScriptInfoForNormalizedPath(file);
            const { position, textRange } = this.extractPositionAndRange(args, scriptInfo);
        ...
        private handlers = createMapFromTemplate<(request: protocol.Request) => { response?: any, responseRequired?: boolean }>({
            ...
            [CommandNames.GetActionsForRefactor]: (request: protocol.Refactor.GetRefactorCodeActionsRequest) => {
                return this.requiredResponse(this.getActionsForRefactor(request.arguments));
            }
        });
